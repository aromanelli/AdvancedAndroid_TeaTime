...
public class OrderSummaryActivityTest {
    ...
    // TODO (3) Finish this method which runs before each test and will
    // stub all external intents so all external intents will be blocked
    @Before
    ...
    // TODO (4) Finish this method which verifies that the intent sent by clicking
    // the send email button matches the intent sent by the application
    @Test
    public void clickSendEmailButton_SendsEmail() {

        onView(withId(R.id.send_email_button)).perform(click());

        // intended(Matcher<Intent> matcher) asserts the given matcher
        // matches one and only one intent sent by the application.
        intended(AllOf.allOf(
                // See OrderSummaryActivity.sendEmail(View) for what params used below to use ...
